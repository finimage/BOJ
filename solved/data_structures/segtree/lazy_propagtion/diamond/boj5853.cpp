...
void res(int start, int end, int ind, int v) {
    lazy_pro(start, end, ind);
    if (tree[ind].Max < v) {
        ans++;
        return;
    ...
    if (tree[ind * 2].Max >= v) res(start, mid, ind * 2, v);
    else if (tree[ind * 2].ri + tree[ind * 2 + 1].le >= v) {
        int L = tree[ind * 2].ri, R = v - tree[ind * 2].ri;
        if (L > 0) update(1, n, 1, tree[ind * 2].riind, mid, 1);
        update(1, n, 1, mid + 1, mid + R, 1);
    }
    else res(mid + 1, end, ind * 2 + 1, v);
}

void solve() {
    int i, a, b; char c; cin >> n >> m;
    init(1, n, 1);
    while (m--) {
        cin >> c >> a;
        if (c == 'A') res(1, n, 1, a);
