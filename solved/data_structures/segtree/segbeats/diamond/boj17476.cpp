...
void lazy_prop(int start, int end, int ind) {
    if (lazy[ind] == pair((ll)0,(ll)0)) return;
    pll k = lazy[ind];
    lazy[ind] = {0,0};
    if (k.x) {
...
Info update_interval2(int start, int end, int ind, int le, int ri) {
    lazy_prop(start, end, ind);
    if (ri < start || end < le) return tree[ind];
    if (le <= start && end <= ri) {
        if ((ll)sqrt(tree[ind].Min) == (ll)sqrt(tree[ind].Max)) {
            lazy[ind].x = (ll)sqrt(tree[ind].Min);
            lazy_prop(start, end, ind);
            return tree[ind];
        }
        if (tree[ind].Min + 1 == tree[ind].Max) {
            lazy[ind].y = (ll)sqrt(tree[ind].Min) - tree[ind].Min;
            lazy_prop(start, end, ind);
            return tree[ind];
        }
    }
    int mid = (start + end) / 2;
    Info l = update_interval2(start, mid, ind * 2, le, ri);
    Info r = update_interval2(mid + 1, end, ind * 2 + 1, le, ri);
    return tree[ind] = merge(l, r);
}
